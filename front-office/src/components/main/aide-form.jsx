import React, { useState } from 'react';
import { API_ENDPOINTS } from '../../config/API_ENDPOINT';
import Loader from './loader-component';
import { Trans, useTranslation } from 'react-i18next';

const AideForm = () => {
      const { t } = useTranslation('');

      const [loading, setLoading] = useState(false);
      const [formData, setFormData] = useState({ nom: '', prenom: '', fichier: null });
      const [message, setMessage] = useState('');
      const [isError, setIsError] = useState(false);

      const handleChange = (e) => {
            const { name, value, files } = e.target;
            setFormData({
                  ...formData,
                  [name]: files ? files[0] : value,
            });
      };

      const handleSubmit = async (e) => {
            e.preventDefault();

            const data = new FormData();
            data.append('name', formData.nom);
            data.append('firstName', formData.prenom);
            data.append('file', formData.fichier);

            setLoading(true);

            try {
                  const response = await fetch(API_ENDPOINTS.fonsejSubmit, {
                        method: 'POST',
                        body: data,
                  });

                  const result = await response.json();

                  if (!response.ok) {
                        setIsError(true);
                        setMessage(result.message || t('form.messages.error'));
                        return;
                  }

                  setIsError(false);
                  setMessage(t('form.messages.success'));

                  setFormData({ nom: '', prenom: '', fichier: null });
            } catch (error) {
                  setIsError(true);
                  setMessage(t('form.messages.genericError'));
                  console.error(error);
            } finally {
                  setLoading(false);
            }
      };

      return (
            <div className="max-w-xl mx-auto mt-10 p-4 lg:p-16 bg-white rounded-2xl shadow-lg">
                  <h2 className="text-2xl font-bold text-start mb-6">{t('form.title')}</h2>

                  {message && (
                        <div
                              className={`p-3 mb-4 rounded-xl ${isError ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}
                        >
                              {message}
                        </div>
                  )}

                  <p className="font-roboto">
                        <Trans
                              i18nKey="form.instructions"
                              ns=""
                              components={{ strong: <strong /> }}
                        />
                  </p>

                  <a
                        href="https://back.fmed.ml/public/uploads/res_telechargable/fiche_fonsej.pdf"
                        className="text-gold hover:text-yellow-300 my-2 mb-4 block"
                        download
                        target="_blank"
                        rel="noopener noreferrer"
                  >
                        <i className="fas fa-file-zipper mr-2"></i> {t('form.downloadFile')}
                  </a>

                  <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="flex flex-col md:flex-row gap-4">
                              <div className="w-full md:w-1/2">
                                    <input
                                          type="text"
                                          name="nom"
                                          placeholder={t('form.placeholder.nom')}
                                          value={formData.nom}
                                          onChange={handleChange}
                                          className="w-full px-4 py-2 border rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                                          required
                                    />
                              </div>

                              <div className="w-full md:w-1/2">
                                    <input
                                          type="text"
                                          name="prenom"
                                          placeholder={t('form.placeholder.prenom')}
                                          value={formData.prenom}
                                          onChange={handleChange}
                                          className="w-full px-4 py-2 border rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                                          required
                                    />
                              </div>
                        </div>

                        <div className="w-full">
                              <p className="font-roboto text-sm text-red-500 mb-4">
                                    {t('form.fileNote')}
                              </p>

                              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                    <label
                                          htmlFor="fichier"
                                          className="bg-black text-white px-4 py-2 rounded-xl cursor-pointer shadow"
                                    >
                                          {t('form.importLabel')}
                                    </label>

                                    <span className="text-black break-all">
                                          {formData.fichier
                                                ? formData.fichier.name
                                                : t('form.noFile')}
                                    </span>
                              </div>

                              <input
                                    type="file"
                                    id="fichier"
                                    name="fichier"
                                    onChange={handleChange}
                                    className="hidden"
                                    required
                              />
                        </div>

                        <div className="flex">
                              <button
                                    type="submit"
                                    className="py-2 px-12 bg-gold text-black font-semibold rounded-xl shadow-md hover:bg-yellow-300 transition"
                              >
                                    {loading ? (
                                          <Loader size={5} color="black" />
                                    ) : (
                                          t('form.button.send')
                                    )}
                              </button>
                        </div>
                  </form>
            </div>
      );
};

export default AideForm;
